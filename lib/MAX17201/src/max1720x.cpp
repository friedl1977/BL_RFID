...
{
	Wire.beginTransmission(MAX1720X_ADDR);
	Wire.write(MAX1720X_RAW_COULOMB_QH_ADDR);
	Wire.endTransmission(false);
	Wire.requestFrom(MAX1720X_ADDR, (int)2,HIGH); //send stop
    uint16_t combined = Wire.read()|(Wire.read()<<8); // LSB or-ed with MSB
    double CoulombsQH = combined; //combine registers
	//double coulombsQH = (double)combined*0.005/0.01;
    return CoulombsQH; // //calculate actual value and return in mA - still TO DO
}

// Returns RAW Coulomb_QL count of the connected LiIon Polymer battery
double max1720x::getCoulombsQL()
{
	Wire.beginTransmission(MAX1720X_ADDR);
	Wire.write(MAX1720X_RAW_COULOMB_QL_ADDR);
	Wire.endTransmission(false);
	Wire.requestFrom(MAX1720X_ADDR, (int)2,HIGH); //send stop
    uint16_t combined = Wire.read()|(Wire.read()<<8); // LSB or-ed with MSB
    double CoulombsQL = combined; //combine registers
	//double coulombsQL = (double)combined*0.005/0.01;
